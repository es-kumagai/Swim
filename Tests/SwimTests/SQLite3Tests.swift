...
extension MyData2 : SQLite3Translateable {
    ...
    static var sqlite3Columns: [Column] {
        ...
        Column("name", keyPath: \.name)
        Column("option", keyPath: \.option)
        Column(.init("id", function: "SUM", alias: "SUM"))
    }
...
class SQLite3Tests: XCTestCase {
    ...
    func testTranslate() throws {
        ...
        XCTAssertEqual(metadata.map { $0.bindingTo?.datatype }, [.integer, .real, .text, .variant])
        XCTAssertEqual(metadata.map { $0.bindingTo?.nullable }, [false, true, false, true])
        ...
        let selectSQL22 = SQLite3.SQL.select(from: MyData2.self)
        
        XCTAssertEqual(selectSQL1.description, #"SELECT "id", "flags", "name", "option" FROM "MyData""#)
        XCTAssertEqual(selectSQL2.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("id" BETWEEN 3 AND 5)"#)
        XCTAssertEqual(selectSQL3.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("name" = 'TEST')"#)
        XCTAssertEqual(selectSQL4.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("id" < 3)"#)
        XCTAssertEqual(selectSQL5.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("id" <= 3)"#)
        XCTAssertEqual(selectSQL6.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("id" > 3)"#)
        XCTAssertEqual(selectSQL7.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("id" >= 3)"#)
        XCTAssertEqual(selectSQL8.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("id" BETWEEN 3 AND 5)"#)
        XCTAssertEqual(selectSQL9.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE (("id" NOT BETWEEN 3 AND 5) AND ("name" = 'TEST'))"#)
        XCTAssertEqual(selectSQL10.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE (("id" NOT BETWEEN 3 AND 5) AND ("name" = 'TEST') OR ("option" < 10))"#)
        XCTAssertEqual(selectSQL11.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE (("id" NOT BETWEEN 3 AND 5) AND (("name" = 'TEST') OR ("option" < 10)))"#)
        XCTAssertEqual(selectSQL12.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE (("id" NOT BETWEEN 3 AND 5) AND ("name" = 'TEST') AND ("option" < 10))"#)
        XCTAssertEqual(selectSQL13.description, #"SELECT COUNT(*) FROM "MyData""#)
        XCTAssertEqual(selectSQL14.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("option" IS NULL)"#)
        XCTAssertEqual(selectSQL15.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("option" IS NOT NULL)"#)
        XCTAssertEqual(selectSQL16.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("name" IN ('A', 'B', 'C'))"#)
        XCTAssertEqual(selectSQL17.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("id" NOT IN (3, 8, 10))"#)
        XCTAssertEqual(selectSQL18.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("name" LIKE '%TEST%')"#)
        XCTAssertEqual(selectSQL19.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("name" NOT LIKE '%TEST%')"#)
        XCTAssertEqual(selectSQL20.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("name" REGEXP '.*')"#)
        XCTAssertEqual(selectSQL21.description, #"SELECT "id", "flags", "name", "option" FROM "MyData" WHERE ("name" REGEXP BINARY '.*')"#)
        XCTAssertEqual(selectSQL22.description, #"SELECT "id", "flags", "name", "option", SUM("id") AS "SUM" FROM "MyData2""#)
